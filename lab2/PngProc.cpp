...
size_t NPngProc::readPngFile(const char* szFileName, unsigned char* pBuf,
    ...
    try {
         ...
         * functions.  If you want to use the default stderr and longjump
         * method, you can supply NULL for the last three parameters.  We also
         * supply the the compiler header file version, so that we know if the
         * application was compiled with a compatible version of the library.
         * REQUIRED
         */
        png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, 0, PNGError, 0);
        ...
        for (int pass = 0; pass < number_passes; pass++) {
            ...
            for (unsigned int y = 0; y < height; y++) {
                png_read_rows(png_ptr, &pOut, nullptr, 1);
                pOut += nRowSize;
            }
        }
...
size_t NPngProc::readPngFileGray(const char* szFileName, unsigned char* pBuf,
                                 size_t* pWidth, size_t* pHeight) {
    try {
        png_structp png_ptr;
        png_infop info_ptr;
        ...
        if (pixel_depth == 8) {
            ...
            unsigned char* pTempBuf =
                new unsigned char[width * (pixel_depth / 8)];
            if (!pTempBuf)
            ...
            for (int pass = 0; pass < number_passes; pass++) {
                unsigned char* pOut = pBuf;
                for (unsigned int y = 0; y < height; y++) {
                    png_read_rows(png_ptr, &pTempBuf, nullptr, 1);

                    unsigned char* pIn = pTempBuf;
                    for (unsigned int x = 0; x < width; ++x) {
                        *pOut++ =
                            ((77 * *pIn + 150 * *(pIn + 1) + 29 * *(pIn + 2))) /
                            256;
                        pIn += 3;
                    }
                }
...
size_t NPngProc::writePngFile(const char* szFileName, unsigned char* pBuf,
    ...
    try {
        ...
        if (fp == NULL) throw CPNGExeption("can't create png file");
         ...
         * functions.  If you want to use the default stderr and longjump
         * method, you can supply NULL for the last three parameters.  We also
         * check that the library version is compatible with the one used at
         * compile time, in case we are using dynamically linked libraries.
         * REQUIRED.
         */
        png_ptr =
            png_create_write_struct(PNG_LIBPNG_VER_STRING, 0, PNGError, 0);
        ...
        png_set_IHDR(png_ptr, info_ptr, (png_uint_32)nWidth,
                     (png_uint_32)nHeight, bit_depth, nPngColorType,
        ...
        printf("NPngProc:writePngFile: Error - %s", e.m_szErrorMessage);
        return NPngProc::PNG_ERROR;
    }
}